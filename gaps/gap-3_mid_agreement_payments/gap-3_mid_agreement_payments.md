...
- To enable mid-agreement (mid-activity) payments, the Provider may periodically send payable Debit Notes (Debit Notes that have the `payment_due_date` attribute set to a non-empty value) to the requestor.
...
- Both intervals are relative to the agreement timestamp
